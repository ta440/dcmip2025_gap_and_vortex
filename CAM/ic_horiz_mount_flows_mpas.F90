module ic_horiz_mount_flow
  ...
  contains
    ...
    subroutine horiz_mount_flow_set_ic(vcoord, latvals, lonvals, zint, U, V, W, T, PS, PHIS, &
      ...
      real(r8)                          :: h0
      real(r8)                          :: half_pi, z_bar, A_k, ztop
      real(r8)                          :: cos_fact, r_on_a, max_w
      ...
      if (l3d_vars) then
        ...
        if (lw) then
          do k = 1, nlev
            ...
            ! For now, assume we are giving nonzero W for MPAS ... 
            if (vcoord == vc_height) then
              ! Find the computational grid level, as zint far from the topography, i.e. (0,0)
              z_bar = zint(0, k)
              ztop = 20.e3_r8    
              A_k = cos(half_pi*z_bar/ztop)**6.0_r8
              if (test_enum == 1) then
                ! For the gap flow:
                tmp1 = -u0*lon_expon/(rearth*lon_scale)
                half_pi = 2.0_r8*atan(1.0_r8)
                ztop = 20.e3_r8    
                write(iulog,*) ' k = "', k, '"'
                write(iulog,*) ' z_bar"', z_bar, '"'
                !write(iulog,*) ' half_pi"', half_pi, '"'
                write(iulog,*) ' ztop"', ztop, '"'
                write(iulog,*) ' tmp1"', tmp1, '"'
                write(iulog,*) ' A_k"', A_k, '"'

                where(mask_use)
                  tmp(:) = ((lonvals(:) - mount_lon)/lon_scale)**(lon_expon - 1.0_r8)
                  W(:,k) = tmp1*tmp(:)*A_k*surf_h(:)
                end where
                max_w = MAXVAL(W(:,k))
                write(iulog,*) '  W (nonhydrostatic) MAX is "', max_w,'"'
                !write(iulog,*) ' W is computed at level,"',k,'"'
              else
                ! For the vortex shedding
                tmp1 = 2.0_r8*u0/(d*d)
                where(mask_use)
                  r(:) = rearth * acos(sin(mount_lat)*sin(latvals(:))+cos(mount_lat)*cos(latvals(:))*cos(lonvals(:)-mount_lon))
                end where
                do i = 1, ncol
                  ...
                  r_on_a = r(i)/rearth
                  cos_fact = 1-cos(r_on_a)**2.0_r8
                  if (cos_fact .le. 1.e-5_r8) then
                    tmp(i) = 0._r8
                  else
                    tmp(i) = sin(lonvals(i)-mount_lon)*cos(mount_lat)*cos(latvals(i))/(sqrt(cos_fact))
                  end if
                end do
                where(mask_use)
                  W(:,k) = tmp1*tmp(:)*A_k*surf_h(:)
